...
def build_campaign_data_from_xlsx(xlsx_path, sheet_name=None):
    ...
    for row in ws.iter_rows(min_row=header_row+1, values_only=True):
        ...
        campaign_name = campaign_name or str(row[COL['campaign']] or '').strip()

        live_date = row[COL['live_date']]
        views    = safe_float(row[COL['views']])
        likes    = safe_float(row[COL['likes']])
        comments = safe_float(row[COL['comments']])
        clicks   = safe_float(row[COL['link_clicks']])
        eng      = safe_float(row[COL['total_engagement']])
        er_raw   = safe_float(row[COL['engagement_rate']])
        budget   = safe_float(row[COL['budget']])
        cpc      = safe_float(row[COL['cpc']])
        avg_ccv  = safe_float(row[COL['avg_ccv']])
        peak_ccv = safe_float(row[COL['peak_ccv']])
