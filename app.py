import os, re, time, uuid, threading, json
...
def yt_video_stats(video_id):
    """Get stats for a specific YouTube video"""
    try:
        r = req.get("https://www.googleapis.com/youtube/v3/videos",
                    params={"part": "statistics,snippet", "id": video_id, "key": YOUTUBE_API_KEY}, timeout=10)
...
def rebrandly_clicks(link):
    """Get click count for a Rebrandly tracking link"""
    if not link or str(link).strip() in ("", "N/A", "None"): return None
    try:
        link = str(link).strip()
        # Extract slug from URL
        m = re.search(r'rebrand\.ly/([^/?]+)', link)
        if not m: return None
        slug = m.group(1)
        r = req.get(f"https://api.rebrandly.com/v1/links",
                    params={"slashtag": slug},
                    headers={"apikey": REBRANDLY_API_KEY}, timeout=10)
        data = r.json()
        if isinstance(data, list) and data:
            return data[0].get("clicks", 0)
        return None
    except: return None
...
def get_twitch_token():
    ...
    try:
        r = req.post("https://id.twitch.tv/oauth2/token",
                     params={"client_id": TWITCH_CLIENT_ID, "client_secret": TWITCH_SECRET,
...
def twitch_stream_stats(username):
    """Get Twitch stats via TwitchTracker scraping"""
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        r = req.get(f"https://twitchtracker.com/{username.lower()}", headers=headers, timeout=15)
        if r.status_code != 200: return None
        page = r.text
        avg_ccv = re.search(r'Average viewers.*?<span[^>]*>([\d,]+)</span>', page, re.DOTALL)
        peak_ccv = re.search(r'Peak viewers.*?<span[^>]*>([\d,]+)</span>', page, re.DOTALL)
        hours = re.search(r'Hours watched.*?<span[^>]*>([\d,\.]+[KkMm]?)</span>', page, re.DOTALL)
        def parse_num(m):
            if not m: return None
        ...
        return {
            "avg_ccv":  parse_num(avg_ccv),
            "peak_ccv": parse_num(peak_ccv),
            "hours":    parse_num(hours),
        }
    except: return None
...
# ── PROCESS ──────────────────────────────────────────────────────────────────
def process(jid, input_path, output_path, run_yt=True, run_tw=True, run_ig=False, run_tt=False, run_rebrandly=True):
    def log(msg): job_log(jid, msg)

    # Fixed column positions (0-based) matching CPT template:
    # A=Talent, B=Content URL, C=Rebrandly, D=Game, E=Format, F=Platform
    # G=Followers, H=Views, I=7Day Avg CCV/Views, J=Avg CCV, K=Peak CCV
    # L=Hours Watched, M=Likes, N=Comments, O=Shares, P=Saves
    # Q=Total Engagement, R=Engagement Rate, S=Link Clicks, T=%to Goal, U=Budget, V=CPC
    COL = {
        'talent': 0, 'live_date': 1, 'content_url': 2, 'rebrandly': 3,
        'campaign': 4, 'format': 5, 'platform': 6, 'followers': 7,
        'views': 8, 'avg_views': 9, 'avg_ccv': 10, 'peak_ccv': 11,
        'hours': 12, 'likes': 13, 'comments': 14, 'shares': 15, 'saves': 16,
        'total_engagement': 17, 'engagement_rate': 18, 'link_clicks': 19,
        'pct_goal': 20, 'budget': 21, 'cpc': 22
    }
    ...
    try:
        ...
        if not header_row:
            ...
            log("❌ Could not find header row with 'TALENT' column")
            return

        log(f"📋 Template detected, header on row {header_row}")
        ...
        for idx, row in enumerate(rows, 1):
            talent      = row[COL['talent']].value
            content_url = row[COL['content_url']].value
            rebrandly   = row[COL['rebrandly']].value
            platform    = row[COL['platform']].value
            live_date   = row[COL['live_date']].value
            avg_kpi     = row[COL['avg_views']].value  # EST KPI for % to goal calc

            log(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            log(f"[{idx}/{total}] {talent} ({platform})")

            progress = int((idx - 1) / total * 90)
            job_update(jid, progress=progress)

            views = likes = comments = 0

            # YOUTUBE
            if run_yt and platform and 'YOUTUBE' in str(platform).upper():
                ...
                if vid_id:
                    ...
                    if stats:
                        ...
                        row[COL['views']].value    = views
                        row[COL['likes']].value    = likes
                        row[COL['comments']].value = comments
                        # Total engagement = likes + comments + link clicks (shares/saves manual)
                        total_eng = likes + comments
                        row[COL['total_engagement']].value = total_eng
                        # Engagement rate = total engagement / views
                        if views > 0:
                            row[COL['engagement_rate']].value = round(total_eng / views * 100, 2)
                        # % to goal = views / avg KPI
                        if avg_kpi and str(avg_kpi) not in ('-', '', 'None') and float(str(avg_kpi).replace(',','')) > 0:
                            row[COL['pct_goal']].value = round(views / float(str(avg_kpi).replace(',','')) * 100, 1)
                        log(f"      Views: {fmt(views)} | Likes: {fmt(likes)} | Comments: {fmt(comments)} | ER: {row[COL['engagement_rate']].value}%")
                    else:
                        log(f"      ⚠️ Could not fetch video stats")
                    time.sleep(0.3)

            # TWITCH
            if run_tw and platform and 'TWITCH' in str(platform).upper():
                handle = twitch_handle(content_url)
                if not handle:
                    handle = str(talent).lower().replace(' ', '').replace('_','') if talent else None
                if handle:
                    log(f"   🟣 Fetching Twitch stats for {handle}...")
                    stats = twitch_stream_stats(handle)
                    if stats:
                        if stats['avg_ccv']:
                            row[COL['avg_ccv']].value  = stats['avg_ccv']
                            row[COL['avg_views']].value = stats['avg_ccv']
                        if stats['peak_ccv']:
                            row[COL['peak_ccv']].value = stats['peak_ccv']
                        if stats['hours']:
                            row[COL['hours']].value    = stats['hours']
                        log(f"      Avg CCV: {fmt(stats['avg_ccv'])} | Peak: {fmt(stats['peak_ccv'])} | Hours: {fmt(stats['hours'])}")
                    else:
                        log(f"      ⚠️ Could not fetch Twitch stats")
                    time.sleep(0.5)

            # INSTAGRAM (Modash)
            if run_ig and platform and 'INSTAGRAM' in str(platform).upper():
                log(f"   📸 Instagram: Modash API not yet configured")
                # TODO: integrate Modash API when available
                # handle = ig_handle(content_url)
                # data = modash_report("instagram", handle)
                # row[COL['likes']].value = data.get('likes')
                # row[COL['comments']].value = data.get('comments')

            # TIKTOK (Modash)
            if run_tt and platform and 'TIKTOK' in str(platform).upper():
                log(f"   🎵 TikTok: Modash API not yet configured")
                # TODO: integrate Modash API when available
                # handle = tt_handle(content_url)
                # data = modash_report("tiktok", handle)
                # row[COL['views']].value = data.get('views')

            # REBRANDLY
            if run_rebrandly and rebrandly and str(rebrandly).strip() not in ('', 'None', '-'):
                log(f"   🔗 Fetching Rebrandly clicks...")
                clicks = rebrandly_clicks(str(rebrandly).strip())
                if clicks is not None:
                    row[COL['link_clicks']].value = clicks
                    # Update total engagement to include clicks
                    cur_eng = row[COL['total_engagement']].value
                    if cur_eng and str(cur_eng) not in ('-', ''):
                        row[COL['total_engagement']].value = int(cur_eng) + clicks
                    # CPC = budget / clicks
                    budget = row[COL['budget']].value
                    if budget and clicks > 0 and str(budget) not in ('-', ''):
                        try:
                            row[COL['cpc']].value = round(float(str(budget).replace(',','').replace('$','')) / clicks, 2)
                        except: pass
                    log(f"      Link clicks: {fmt(clicks)}")
                else:
                    log(f"      ⚠️ No Rebrandly data (check link format)")
                time.sleep(0.2)

        # Style auto-filled cells in light olive
        from openpyxl.styles import PatternFill
        fill = PatternFill("solid", fgColor="E8F0C8")
        for row in ws.iter_rows(min_row=header_row+1):
            if not row[0].value or str(row[0].value).strip() in ("", "TALENT"):
                continue
            for c in [COL['views'], COL['likes'], COL['comments'], COL['avg_ccv'],
                      COL['peak_ccv'], COL['hours'], COL['total_engagement'],
                      COL['engagement_rate'], COL['link_clicks'], COL['pct_goal'], COL['cpc']]:
                if row[c].value and str(row[c].value) not in ('-', ''):
                    row[c].fill = fill

        wb.save(output_path)

        # Generate PPTX from the filled data
        ...
        try:
            ...
            job_update(jid, status="complete", progress=100, pptx=pptx_path,
                       pptx_name=os.path.basename(pptx_path))
            log("✅ PowerPoint deck generated!")
        except Exception as pe:
            log(f"⚠️ PPTX generation failed: {pe}")
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --green-hill: #5a6e10;
      --grad: linear-gradient(135deg, #4a5a0a 0%, #8a9e20 100%);
      --accent: #c8d840;
    }
    html, body { min-height: 100vh; background: var(--bg); color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 15px; line-height: 1.6; overflow-x: hidden; }
    ...
    .top-bar-badge {
      ...
      color: var(--accent);
      background: rgba(200,216,64,0.12);
    ...
    .hero-eyebrow {
      ...
      padding: 5px 14px; border: 1px solid rgba(74,90,10,0.3);
      border-radius: 100px; background: rgba(74,90,10,0.06);
    }
    .hero h1 {
      font-family: 'Poppins', sans-serif;
    ...
    .toggle-chip:hover { border-color: var(--olive); color: var(--black); }
    .toggle-chip.active {
      border-color: var(--olive);
      background: rgba(74,90,10,0.08);
      color: var(--black);
      font-weight: 600;
    }
    ...
    .drop-zone {
      ...
      transition: all 0.25s; background: rgba(0,0,0,0.02);
      position: relative;
    }
    .drop-zone:hover, .drop-zone.drag-over {
      border-color: var(--olive); background: rgba(74,90,10,0.05);
    }
    ...
    #file-selected {
      ...
      padding: 14px 18px;
      background: rgba(74,90,10,0.06);
      border: 1.5px solid rgba(74,90,10,0.25);
      border-radius: 12px;
    }
    ...
    #file-selected .remove-btn:hover { color: #c0392b; }
    ...
    .btn-primary:hover:not(:disabled) {
      ...
      box-shadow: 0 8px 24px rgba(74,90,10,0.4);
    }
    .btn-primary:disabled { opacity: 0.35; cursor: not-allowed; transform: none; }
    .btn-outline {
      background: transparent; border: 1.5px solid var(--border);
      color: var(--muted); width: 100%; padding: 10px 16px;
    ...
    .complete-icon {
      ...
      margin: 0 auto 20px;
      box-shadow: 0 4px 24px rgba(74,90,10,0.35);
    }
    .complete-title {
      font-family: 'Poppins', sans-serif; font-size: 26px;
      font-weight: 800; text-align: center; margin-bottom: 10px;
    }
    .complete-sub { text-align: center; color: var(--muted); font-size: 14px; margin-bottom: 28px; }

    .template-links { text-align: center; margin-top: 14px; font-size: 12px; color: var(--muted); }
    .template-links a { color: var(--muted); text-decoration: none; transition: color 0.2s; }
...
<body>
  ...
  <div class="page">
    <div id="main-screen">
      <div class="hero">
        ...
        <p>Upload your campaign spreadsheet and we'll auto-fill views, engagement, and Rebrandly click data — ready for your EOC deck in minutes.</p>
      </div>
      ...
      <div class="card">
        <div class="platform-section">
          ...
          <div class="platform-toggles">
            <div class="toggle-chip active" id="toggle-yt" onclick="togglePlatform('yt')" style="cursor:pointer;">
              <input type="checkbox" id="check-yt" checked>
              <span>🎬 YouTube</span>
            </div>
            <div class="toggle-chip active" id="toggle-tw" onclick="togglePlatform('tw')" style="cursor:pointer;">
              <input type="checkbox" id="check-tw" checked>
              <span>🟣 Twitch</span>
            </div>
            <div class="toggle-chip" id="toggle-ig" onclick="togglePlatform('ig')" style="cursor:pointer;">
              <input type="checkbox" id="check-ig">
              <span>📸 Instagram</span>
            </div>
            <div class="toggle-chip" id="toggle-tt" onclick="togglePlatform('tt')" style="cursor:pointer;">
              <input type="checkbox" id="check-tt">
              <span>🎵 TikTok</span>
            </div>
            <div class="toggle-chip active" id="toggle-rb" onclick="togglePlatform('rb')" style="cursor:pointer;">
              <input type="checkbox" id="check-rb" checked>
              <span>🔗 Rebrandly</span>
            </div>
          </div>
          <div style="margin-top:10px;font-size:12px;color:var(--muted);opacity:0.7;">💡 Instagram & TikTok require Modash API — enable when available</div>
        </div>
        ...
        <div class="drop-zone" id="drop-zone"
          ...
          <div class="drop-text">
            <strong>Drop your campaign report here</strong>
    ...
    <div id="complete-screen">
      ...
      <div class="card">
        ...
        <div class="complete-sub">Campaign data has been pulled. Download your filled report below.</div>
  ...
  function onDrop(e) { e.preventDefault(); document.getElementById('drop-zone').classList.remove('drag-over'); var file = e.dataTransfer.files[0]; if (file) setFile(file); }
  ...
  async function runJob() {
    ...
    var btn = document.getElementById('run-btn-text');
    ...
    try {
      ...
      if (data.job_id) {
        ...
        btn.textContent = 'Pull Campaign Data';
        document.getElementById('run-btn').disabled = false;
      }
    } catch(e) {
  ...
  async function pollStatus() {
    ...
    try {
      ...
      document.getElementById('progress-label').textContent = data.status === 'complete' ? 'Complete!' : data.status === 'error' ? 'Error occurred' : 'Fetching stats...';
      var box = document.getElementById('log-box');
      if (data.log && data.log.length > 0) {
        box.innerHTML = data.log.map(function(l){ return '<span>' + l.replace(/</g,'&lt;') + '</span>'; }).join('<br>');
        box.scrollTop = box.scrollHeight;
      }
      if (data.status === 'complete') {
        clearInterval(pollInterval); pollInterval = null;
        setTimeout(function(){ document.getElementById('progress-screen').style.display='none'; document.getElementById('complete-screen').style.display='block'; }, 1000);
  ...
  async function downloadFile() {
    try {
      ...
      a.href = url; a.download = 'CPT_EOC_Report_Filled.xlsx';
  ...
  async function downloadPPTX() {
    try {
      ...
      if (!res.ok) { alert('PowerPoint not available yet.'); return; }
      var blob = await res.blob();
      var url = window.URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.href = url; a.download = 'CPT_EOC_Deck.pptx';
      document.body.appendChild(a); a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
    } catch(e) { alert('Download error.'); }
...
def run_job():
    ...
    out  = os.path.join(OUTPUT_FOLDER, f"{jid}_{base}_Filled.xlsx")
    f.save(inp)
    job_create(jid, {"status": "running", "progress": 0, "log": [], "output": out, "name": f"{base}_Filled.xlsx", "ts": time.time()})
    t = threading.Thread(target=process, args=(jid, inp, out, run_yt, run_tw, run_ig, run_tt, run_rb), daemon=True)
    t.start()
...
def job_status(jid):
    ...
    return jsonify({"status": j["status"], "progress": j["progress"], "log": j["log"]})
...
def download_pptx(jid):
    ...
    return send_file(pptx, as_attachment=True, download_name=j.get("pptx_name", "CPT_EOC_Deck.pptx"))
