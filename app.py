...
# ── PROCESS ──────────────────────────────────────────────────────────────────
def process(jid, input_path, output_path, run_yt=True, run_tw=True, run_rebrandly=True):
    ...
    try:
        ...
        for cell in ws[header_row]:
            if cell.value:
                h = str(cell.value).strip().upper().split('\n')[0]
                headers[h] = cell.column - 1  # 0-based

        log(f"📋 Found columns: {list(headers.keys())[:8]}...")

        # Get data rows
        rows = [list(ws.iter_rows(min_row=header_row+1, max_row=header_row+1))[0]]
        ...
        col = headers  # shorthand

        for idx, row in enumerate(rows, 1):
            talent = row[0].value
            platform = row[col.get('PLATFORM', 5)].value if 'PLATFORM' in col else None
            content_url = row[col.get('LINK TO CONTENT', 1)].value if 'LINK TO CONTENT' in col else None
            tracking_url = row[col.get('REBRANDLY TRACKING LINK', 2)].value if 'REBRANDLY TRACKING LINK' in col else None
            ...
            if run_yt and platform and 'YOUTUBE' in str(platform).upper():
                ...
                if vid_id:
                    ...
                    if stats:
                        if 'VIEWS' in col: row[col['VIEWS']].value = stats['views']
                        if 'LIKES' in col: row[col['LIKES']].value = stats['likes']
                        if 'COMMENTS' in col: row[col['COMMENTS']].value = stats['comments']
                        log(f"      Views: {fmt(stats['views'])} | Likes: {fmt(stats['likes'])} | Comments: {fmt(stats['comments'])}")
                    else:
            ...
            if run_tw and platform and 'TWITCH' in str(platform).upper():
                ...
                if not handle:
                    # Try talent name as handle
                    handle = str(talent).lower().replace(' ', '') if talent else None
                if handle:
                    log(f"   🟣 Fetching Twitch stats for {handle}...")
                    stats = twitch_stream_stats(handle)
                    if stats:
                        if 'AVG. CCV' in col and stats['avg_ccv']: row[col['AVG. CCV']].value = stats['avg_ccv']
                        if 'PEAK CCV' in col and stats['peak_ccv']: row[col['PEAK CCV']].value = stats['peak_ccv']
                        if 'HOURS WATCHED' in col and stats['hours']: row[col['HOURS WATCHED']].value = stats['hours']
                        log(f"      Avg CCV: {fmt(stats['avg_ccv'])} | Peak: {fmt(stats['peak_ccv'])}")
                    else:
                        log(f"      ⚠️ Could not fetch Twitch stats")
                    time.sleep(0.5)

            # REBRANDLY
            if run_rebrandly and tracking_url:
                log(f"   🔗 Fetching link clicks...")
                clicks = rebrandly_clicks(tracking_url)
                if clicks is not None:
                    if 'LINK CLICKS' in col: row[col['LINK CLICKS']].value = clicks
        ...
        for row in ws.iter_rows(min_row=header_row+1):
            for cell in row:
                if cell.value and cell.value != row[0].value:
                    cell.fill = blue_fill

        wb.save(output_path)
        job_update(jid, status="complete", progress=100)
        log("✅ Complete! Your report is ready to download.")
  ...
  <div class="page">
    <div id="main-screen">
      ...
      <div class="card">
        <div class="platform-section">
          ...
          <div style="margin-top:10px;font-size:12px;color:var(--muted);opacity:0.7;">💡 Only enable platforms relevant to this campaign</div>
        </div>
        ...
        <div class="template-links">
          <a href="https://docs.google.com/spreadsheets/d/1nQDKHV9GcWCWXVDyY_-wSNUOQCHPXV9au172VLIvI-Y/copy" target="_blank">📥 Download EOC template</a>
          &nbsp;·&nbsp;
          <a href="https://docs.google.com/spreadsheets/d/1nQDKHV9GcWCWXVDyY_-wSNUOQCHPXV9au172VLIvI-Y/copy" target="_blank">📊 Open in Google Sheets</a>
        </div>
      </div>
    </div>
    ...
    <div id="complete-screen">
      ...
      <div class="card">
        ...
        <button class="btn btn-primary" onclick="downloadFile()">⬇️ &nbsp;Download Filled Report</button>
...
def run_job():
    ...
    t = threading.Thread(target=process, args=(jid, inp, out, run_yt, run_tw, run_rb), daemon=True)
    t.start()
